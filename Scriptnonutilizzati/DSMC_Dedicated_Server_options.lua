...
DSMC_AutosaveProcess_min		= 10		-- minutes, number, from 2 to 480. DSMC_AutosaveProcess must be true.

DSMC_24_7_serverStandardSetup   = false         -- multiple valid values. This option is a simplified setup for the specific server autosave layout. You can input:
...
DSMC_AutosaveExit_hours			= 6       -- value, 1->24 or 25. hours of simulation after with DCS will close the mission, from 0 to 24 (higher values won't be accepted). If clients are online, it will delay 5 minutes and so on till nobody is online.
...
DSMC_restartCampaign            = false     -- true / false. If true, DSMC will check DCS "mission goals" and when reached, instead of restarting the saved mission, will specifically look for the "_000" miz file and load that one, to restart the campaign
...
DSMC_STTS_SRSpath               = "C:\\Program Files\\DCS-SimpleRadio-StandaloneNEW3"     -- path of the SRS installation, and of DCS-SR-ExternalAudio.exe file. !BEWARE!: errors happens when this is not properly configured
